...
CREATE POLICY "Users can view own quotes" ON public.quotes FOR SELECT USING (auth.uid()::text = user_id::text OR auth.jwt() ->> 'role' = 'admin');
CREATE POLICY "Users can create quotes" ON public.quotes FOR INSERT WITH CHECK (auth.uid()::text = user_id::text);
